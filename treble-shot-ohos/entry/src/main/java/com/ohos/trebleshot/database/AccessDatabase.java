...
public class AccessDatabase extends SQLiteDatabase {
    ...
    public static CursorItem convertValues(ValuesBucket values) {
        CursorItem cursorItem = new CursorItem();

        for (String key : values.getColumnSet())
            cursorItem.put(key, values.getObject(key));
    ...
    public void onUpgrade(RdbStore database, int old, int current) {
        ...
        if (old <= 5) {
            ...
            if (old <= 6) {
                ...
                database.executeSql(String.format("DROP TABLE IF EXISTS `%s`", groupTable.getName()));
                database.executeSql(String.format("DROP TABLE IF EXISTS `%s`", devicesTable.getName()));
            ...
            if (old < 10) {
                ...
                try {
                    ...
                    Map<Long, String> mapDist = new HashMap<>();
//                    new
                    ...
                    database.executeSql(String.format("DROP TABLE IF EXISTS `%s`", tableTransfer
                            .getName()));
            ...
            if (old < 12) {
                ...
                database.executeSql(String.format("DROP TABLE IF EXISTS `%s`", tableTransferGroup
                        .getName()));
    ...
    protected void broadcast(RdbStore database,
                             SQLQuery.Select select, String type) {
//        getContext().sendBroadcast(new ExtIntent(ACTION_DATABASE_CHANGE)
//                .setParam(EXTRA_TABLE_NAME, select.tableName)
//                .setParam(EXTRA_CHANGE_TYPE, type)
//                .setParam(EXTRA_AFFECTED_ITEM_COUNT, getAffectedRowCount(database)));
    }
    ...
    public long getAffectedRowCount(RdbStore database) {
        ResultSet rs;
        long returnCount = 0;

        rs = database.querySql("SELECT changes() AS affected_row_count", null);

        if (rs != null && rs.getRowCount() > 0 && rs.goToFirstRow())
            returnCount = rs.getLong(rs.getColumnIndexForName("affected_row_count"));
    ...
    @Override
    public long insert(RdbStore database, String tableName, String nullColumnHack, ValuesBucket valuesBucket) {
        long returnedItems = super.insert(database, tableName, nullColumnHack, valuesBucket);
    ...
    @Override
    public <T, V extends DatabaseObject<T>> void insert(RdbStore openDatabase, List<V> objects, ProgressUpdater updater, T parent) {
    ...
    @Override
    public int remove(RdbStore database, SQLQuery.Select select) {
    ...
    @Override
    public <T, V extends DatabaseObject<T>> void remove(RdbStore openDatabase, List<V> objects, ProgressUpdater updater, T parent) {
    ...
    public void removeAsynchronous(AbilitySlice activity, final DatabaseObject object) {
    ...
    public void removeAsynchronous(AbilitySlice abilitySlice, final List<? extends DatabaseObject> objects) {
        removeAsynchronous(abilitySlice, new Runnable() {
            @Override
            public void run() {
                remove(objects);
            }
    ...
    public void removeAsynchronous(AbilitySlice activity, final Runnable runnable) {
...
//                runnable.run();
//            }
//        }.setTitle(activity.getString(ResourceTable.String_mesg_removing))
//                .run(activity);
    }
    ...
    @Override
    public <T, V extends DatabaseObject<T>> void update(RdbStore openDatabase, List<V> objects, ProgressUpdater updater, T parent) {
