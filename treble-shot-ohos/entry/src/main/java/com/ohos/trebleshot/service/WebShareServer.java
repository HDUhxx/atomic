...
public class WebShareServer extends NanoHTTPD {
    ...
    private final HiLogLabel label = new HiLogLabel(HiLog.LOG_APP,0x333,WebShareServer.class.toString());

    private final Context mContext;
    private final AccessDatabase database;
    private SystemCapability.Notification notification;
    public WebShareServer(Context context, int port) throws IOException {
        super(port);
        mContext = context;
        database = AppUtils.getDatabase(context);
        mAssetManager = context.getResourceManager();
    ...
    public Response serve(IHTTPSession session) {
        ...
        if ( session.getParms().containsKey("file")) {
            ...
            if (fileName == null || filePath == null || fileName.length() < 1) {
                ...
                return newFixedLengthResponse(Response.Status.ACCEPTED, "text/html",
                        makePage("arrow-left.svg", ResourceTable.String_text_send,
                                makeNotFoundTemplate(ResourceTable.String_mesg_somethingWentWrong,
                                        ResourceTable.String_text_listEmptyFiles)));
            } else {
                File sourceFile = new File(filePath);
                File destFile = FileUtils.getUploadFile(mContext,fileName);
...
//                    CommunicationService.class).setAction(CommunicationService.ACTION_TOGGLE_WEBSHARE));
            Intent intent = new Intent();
                ...
                mContext.startAbility(intent,102);
        }
    ...
    private Response serveAPK() {
        try {
            File file = new File(mContext.getApplicationInfo().getEntryDir());
        ...
        return newFixedLengthResponse(Response.Status.ACCEPTED, "text/html",
                makePage("arrow-left.svg", ResourceTable.String_text_downloads,
                        makeNotFoundTemplate(ResourceTable.String_text_empty,
                                ResourceTable.String_text_webShareNoContentNotice)));
    }
    ...
    private Response serveFileDownload(String[] args, IHTTPSession session) {
        try {
            if ("download".equals(args[0])) {
                ...
                File file = new File(object.file);
    ...
    private String serveMainPage() {
        ...
        for (TransferGroup group : groupList) {
            ...
            contentBuilder.append(makeContent("list_transfer_group", mContext.getString(ResourceTable.String_mode_itemCountedDetailed,
                    String.format("%d files", index.outgoingCount), FileUtils.sizeExpression(index.outgoing,false)),
                    ResourceTable.String_butn_show, "show", group.groupId));
        }

        if (contentBuilder.length() == 0)
            contentBuilder.append(makeNotFoundTemplate(ResourceTable.String_text_listEmptyTransfer,
                    ResourceTable.String_text_webShareNoContentNotice));

        return makePage("icon.png", ResourceTable.String_text_transfers, contentBuilder.toString());
    }
    ...
    private String serveTransferPage(String[] args) {
        try {
            ...
            Log.e(""+groupList.size());
            if (groupList.size() > 0) {
                contentBuilder.append(makeContent("list_transfer",
                        mContext.getString(ResourceTable.String_butn_downloadAllAsZip), ResourceTable.String_butn_download, "download-zip", group.groupId,
                        String.format("%d files", groupList.size()) + ".zip"));

                for (TransferObject object : groupList)
                    contentBuilder.append(makeContent("list_transfer",
                            object.friendlyName + " " + FileUtils.sizeExpression(object.fileSize,
                                    false), ResourceTable.String_butn_download, "download", object.groupId,
                            object.requestId, object.friendlyName));
            }

            return makePage("arrow-left.svg", ResourceTable.String_text_files, contentBuilder.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }

        return makePage("arrow-left.svg", ResourceTable.String_text_files,
                makeNotFoundTemplate(ResourceTable.String_text_listEmptyFiles,
                        ResourceTable.String_text_webShareNoContentNotice));
    }

    private String serveHelpPage() {
        Map<String, String> values = new HashMap<>();
        values.put("help_title", mContext.getString(ResourceTable.String_butn_help));
        values.put("licence_text", Tools.escapeHtml(mContext.getString(ResourceTable.String_conf_licence)));



        return makePage("arrow-left.svg", ResourceTable.String_butn_help, applyPattern(
                getFieldPattern(), readPage("help.html"), values));
    }

    private String makeContent(String pageName, String content,  int buttonRes,
                               Object... objects) {
    ...
    private String makeNotFoundTemplate( int msg,  int detail) {
        Map<String, String> values = new HashMap<>();
    ...
    private String makePage(String image, int titleRes, String content) {
        ...
        values.put("header", appName);
        values.put("title_header", title);
        values.put("main_content", content);
        values.put("help_icon", "/image/help-circle.svg");
        values.put("help_alt", mContext.getString(ResourceTable.String_butn_help));
        values.put("username", "user_name");//todo
        values.put("footer_text", "This software is developed by the Genonbeta Libre Software Org");
    ...
    private InputStream openFile(String fileName) throws IOException {
        return mAssetManager.getRawFileEntry("resources/rawfile/webshare" + File.separator + fileName).openRawFile();
    }
    ...
    protected class ZipBundleResponse extends NanoHTTPD.Response {
        ...
        protected void send(OutputStream outputStream) {
            ...
            try {
                ...
                HiLog.debug(label,"Could not send response to the client"+ioe.toString());
            }
        }
        ...
        private void sendBody(OutputStream outputStream) throws IOException {
            ...
            for (TransferObject object : mFiles) {
                try {
                    ...
                    thisEntry.setTime(file.lastModified());
