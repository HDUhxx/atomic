...
public class TransferObject
    ...
    public String file;
    public String fileMimeType=".txt";
    ...
    public ValuesBucket getValues() {
        ...
        values.putLong(AccessDatabase.FIELD_TRANSFER_ID, requestId);
        values.putLong(AccessDatabase.FIELD_TRANSFER_GROUPID, groupId);
        values.putString(AccessDatabase.FIELD_TRANSFER_DEVICEID, deviceId);
        values.putString(AccessDatabase.FIELD_TRANSFER_NAME, friendlyName);
        values.putLong(AccessDatabase.FIELD_TRANSFER_SIZE, fileSize);
        values.putString(AccessDatabase.FIELD_TRANSFER_MIME, fileMimeType);
        values.putString(AccessDatabase.FIELD_TRANSFER_FLAG, flag.toString());
        values.putString(AccessDatabase.FIELD_TRANSFER_TYPE, type.toString());
        values.putString(AccessDatabase.FIELD_TRANSFER_FILE, file);
        values.putInteger(AccessDatabase.FIELD_TRANSFER_ACCESSPORT, accessPort);
        values.putLong(AccessDatabase.FIELD_TRANSFER_SKIPPEDBYTES, skippedBytes);
        values.putString(AccessDatabase.FIELD_TRANSFER_DIRECTORY, directory);
    ...
    @Override
    public void onCreateObject(RdbStore dbInstance, SQLiteDatabase database, TransferGroup parent) {

    }

    @Override
    public void onUpdateObject(RdbStore dbInstance, SQLiteDatabase database, TransferGroup parent) {

    }

    @Override
    public void onRemoveObject(RdbStore dbInstance, SQLiteDatabase database, TransferGroup parent) {
...
//                file.delete();
            File file = new File(parent.savePath);
            if(file.isFile()){
                file.delete();
            }
        } catch (Exception e) {
