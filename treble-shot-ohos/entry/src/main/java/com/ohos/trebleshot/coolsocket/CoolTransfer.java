package com.ohos.trebleshot.coolsocket;
...
public abstract class CoolTransfer<T>
{
	...
	public abstract static class TransferHandler<T> implements Runnable
	{
		...
		private final ParentBuilder<T> mParentBuilder;
	...
	public static abstract class Receive<T> extends CoolTransfer<T>
	{
		...
		public EventHandler prepare(Builder<T> builder) {
			return new EventHandler(builder);
		}

		public EventHandler receive(Builder<T> builder, boolean currentThread)
		{
			return receive(prepare(builder), currentThread);
		}

		public EventHandler receive(EventHandler handler, boolean currentThread)
		{
			if (currentThread)
				handler.run();
			else
		...
		public class EventHandler extends TransferHandler<T>
		{
			public EventHandler(Builder<T> builder)
			{
				super(builder);
			}
	...
	public static abstract class Send<T> extends CoolTransfer<T>
	{
		...
		public EventHandler send(Builder<T> builder, boolean currentThread)
		{
			return send(prepare(builder), currentThread);
		}

		public EventHandler send(EventHandler handler, boolean currentThread)
		{
			if (currentThread)
				handler.run();
			else
