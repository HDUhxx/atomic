...
public class TransferObjectTest {
    ...
    private final String tableName = AccessDatabase.DIVIS_TRANSFER;
    ...
    public void test_01_InsertOne() {
        ...
        object.requestId = System.currentTimeMillis();
        object.groupId = 1;
        object.file = "xxx";
        object.skippedBytes = System.currentTimeMillis()/1000;
        object.friendlyName = "1";
    ...
    public void test_04_Delete(){
       ...
       testCount(1);
       //删除方式2
       database.remove(new SQLQuery.Select(tableName).setWhere("name=?","1"));
       testCount(0);
    }
    ...
    public void test_03_modify(){
        ...
        p.friendlyName = "2";
        database.publish(p);
        int num2= database.getTable(new SQLQuery.Select(tableName).setWhere("name=?","2")).size();
