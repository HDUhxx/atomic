...
//                        mPaint.getTextBounds(s.getTitle(), 0, s.getTitle().length(), textBounds);
                        legendWidth = Math.max(legendWidth, mPaint.getTextBounds(s.getTitle()).getWidth());
                    }
                }
        ...
        // rect
        float legendHeight = (mStyles.textSize + mStyles.spacing) * removeDuplicate(allSeries).size()- mStyles.spacing;
        float lLeft;
        float lTop;
        ...
        mPaint.setColor(new Color(mStyles.backgroundColor));


        RectFloat rectF = new RectFloat(lLeft, lTop, lRight, lBottom);
        canvas.drawRoundRect(rectF, 8, 8, mPaint);

        int i = 0;


        for (Series series : removeDuplicate(allSeries)) {
            mPaint.setColor(new Color(series.getColor()));
            canvas.drawRect(new RectFloat(lLeft + mStyles.padding, lTop + mStyles.padding + (i * (mStyles.textSize + mStyles.spacing)), lLeft + mStyles.padding + shapeSize, lTop + mStyles.padding + (i * (mStyles.textSize + mStyles.spacing)) + shapeSize), mPaint);
            if (series.getTitle() != null) {
                mPaint.setColor(new Color(mStyles.textColor));
                //右侧提示语
                canvas.drawText(mPaint, series.getTitle(), lLeft + mStyles.padding + shapeSize + mStyles.spacing, lTop + mStyles.padding + mStyles.textSize + (i * (mStyles.textSize + mStyles.spacing)) - 5);
            }
            i++;
        }
    }
     ...
     * @return
     */
    private List<Series> removeDuplicate(List<Series> allSeries) {
        for (int l = 0; l < allSeries.size() - 1; l++) {
            for (int i = allSeries.size() - 1; i > l; i--) {
                if (allSeries.get(i).getTitle().equals(allSeries.get(l).getTitle())) {
                     allSeries.remove(i);
                }
            }
        }
