...
import com.example.doodle.core.*;
...
public abstract class DoodleItemBase implements IDoodleItem, IDoodleItemListener {
    ...
    public void setSize(float size) {
        mSize = size;
        onPropertyChanged(IDoodleItemListener.PROPERTY_SIZE);
        refresh();
    }
    ...
    public void setColor(IDoodleColor color) {
        mColor = color;
        onPropertyChanged(IDoodleItemListener.PROPERTY_COLOR);
        refresh();
    }

    @Override
    public void draw(Canvas canvas, PixelMap pixelMaps) {
        drawBefore(canvas);

        int count = canvas.save();
        mLocation = getLocation(); // 获取旋转后的起始坐标
        canvas.translate(mLocation.getPointX(), mLocation.getPointY()); // 偏移，把坐标系平移到item矩形范围
        float px = mPivotX - mLocation.getPointX(), py = mPivotY - mLocation.getPointY(); // 需要减去偏移
    ...
    public void setLocation(float x, float y, boolean changePivot) {
        float diffX = x - mLocation.getPointX(), diffY = y - mLocation.getPointY();
        mLocation.modify(x, y);
        onPropertyChanged(IDoodleItemListener.PROPERTY_LOCATION);

        if (changePivot) {
            mPivotX = mPivotX + diffX;
            mPivotY = mPivotY + diffY;
            onPropertyChanged(IDoodleItemListener.PROPERTY_PIVOT_X);
            onPropertyChanged(IDoodleItemListener.PROPERTY_PIVOT_Y);
        }
    ...
    public void setItemRotate(float degree) {
        mItemRotate = degree;
        onPropertyChanged(IDoodleItemListener.PROPERTY_ROTATE);
        refresh();
    }
    ...
    @Override
    public void setNeedClipOutside(boolean isClip) {
        mIsNeedClipOutside = isClip;
    }
    ...
    public void setScale(float scale) {
        ...
        mScale = scale;
        onPropertyChanged(IDoodleItemListener.PROPERTY_SCALE);
        refresh();
    }
