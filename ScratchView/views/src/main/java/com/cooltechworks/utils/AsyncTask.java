 ...
 *
 * <p>AsyncTask is designed to be a helper class around {@link Thread} and {@link EventHandler}
 ...
 *
 * @deprecated Use the standard <code>java.util.concurrent</code> utilities</a> instead.
 */
@Deprecated
public abstract class AsyncTask<Params, Progress, Result> {
    private static final HiLogLabel LABEL = new HiLogLabel(HiLog.LOG_APP, 0x00101, "AsyncTask");
    ...
    private static final RejectedExecutionHandler sRunOnSerialPolicy =
            new RejectedExecutionHandler() {
                public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
                    HiLog.warn(LABEL, "Exceeded ThreadPoolExecutor pool size");
    ...
    public AsyncTask(EventRunner callbackLooper) {
        mHandler = callbackLooper == null || callbackLooper == EventRunner.getMainEventRunner()
                ? getMainHandler()
    ...
    private Result postResult(Result result) {
        ...
        event.eventId = MESSAGE_POST_RESULT;
        event.object = new AsyncTaskResult<Result>(this, result);
        getHandler().sendEvent(event, EventHandler.Priority.IMMEDIATE);
        return result;
    }
    ...
    protected final void publishProgress(Progress... values) {
        if (!isCancelled()) {
            InnerEvent event = InnerEvent.get();
            event.eventId = MESSAGE_POST_PROGRESS;
            event.object = new AsyncTaskResult<Progress>(this, values);
            getHandler().sendEvent(event, EventHandler.Priority.IMMEDIATE);
        }
    }
    ...
    private static class InternalHandler extends EventHandler {
        public InternalHandler(EventRunner runner) {
            super(runner);
        }
